...
fn init_test_env() {
    INIT.call_once(|| {
        ...
        std::env::set_var("DATABASE_URL", "postgresql://postgres:123456@localhost:5432/aicom_w1_project_alpha");
...
async fn test_ticket_filtering() {
    ...
    // 1. 按标签筛选
    let resp = client
        .get(&format!("/api/tickets?tag={}", tag_id))
        .await;
    assert_eq!(resp.status(), 200);
    let tickets: Vec<serde_json::Value> = resp.json().await.unwrap();
    ...
    // 6. 组合筛选：标签 + 完成状态 + 搜索
    let resp = client
        .get(&format!("/api/tickets?tag={}&completed=true&search=bug", tag_id))
        .await;
    assert_eq!(resp.status(), 200);
    let tickets: Vec<serde_json::Value> = resp.json().await.unwrap();
    assert_eq!(tickets.len(), 1);
...
async fn test_error_handling() {
    ...
    let resp = client
        .delete(&format!("/api/tickets/{}/tags/00000000-0000-0000-0000-000000000000", ticket_id))
        .await;
    assert_eq!(resp.status(), 404);
}
