...
const DatabaseMenuItem: React.FC<DatabaseMenuItemProps> = ({
  db,
  onEdit,
  onDelete,
}) => {
...
export const AppLayout: React.FC = () => {
  ...
  const menuItems = databases.map((db) => ({
    ...
    label: (
      <DatabaseMenuItem db={db} onEdit={handleEdit} onDelete={handleDelete} />
    ),
  }));
    ...
    <Layout style={{ minHeight: "100vh" }}>
      ...
      <Layout>
        ...
        <Content style={{ background: "#fff", marginLeft: 16 }}>
          {selectedDbId ? (
            <DatabaseExplorer selectedDbId={selectedDbId} />
          ) : (
